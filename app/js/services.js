...
.factory('Autoupdater', function($http, ngDialog) {
	...
	var fs = require('fs'),
		...
		updatePlist = function(version) {
			var plistPath = path.join(process.cwd(), '..', '..', 'Info.plist'),
				xml = fs.readFileSync(plistPath, 'utf-8'),
				key = 'CFBundleShortVersionString</key>\n\t<string>',
				index = xml.indexOf(key) + key.length,
				plist = xml.substr(0, index) + 'Version ' + version + '</string>\n\t<key>CFBundleVersion</key>\n\t<string>' + version;

			plist += xml.substr(xml.indexOf('</string>', xml.indexOf('</string>', index) + 1));
			fs.writeFileSync(plistPath, plist + xml);
		};
	...
	return {
		...
		run: function() {
			var self = this;
			$http.get('http://updates.gatunes.com/latest.json?' + (new Date() * 1)).then(function(response) {
				var release = response.data;
				if(response.status !== 200 || !release || versionCompare(self.currentVersion, release.version) >= 0) return;
				var packageUrl = release.pkg[process.platform] || release.pkg;
				http.get(packageUrl, function(res) {
					var data;
					res.on('data', function(chunk) {
						data = data ? Buffer.concat([data, chunk]) : chunk;
					});
					res.on('end', function() {
						if(crypto.createHash('sha256').update(data).digest('hex') !== release.signature) return;
						if(process.platform === 'darwin') {
							new (require('adm-zip'))(data).extractAllTo(process.cwd(), true);
							updatePlist(release.version);
						} else fs.writeFileSync(packagePath, data);
						self.currentVersion = release.version;
						ngDialog.open({
							template: 'dialogs/updated.html',
							controller: 'updated',
							showClose: false,
